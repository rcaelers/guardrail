...
impl AttachmentsRepo {
    pub async fn get_by_id(
        ...
        let mut result = db
            .query("SELECT *, meta::id(id) as id, meta::id(crash_id) as crash_id, meta::id(product_id) as product_id FROM ONLY type::record('attachments', $id)")
    ...
    pub async fn get_all(
        ...
        let query = format!(
            "SELECT *, meta::id(id) as id, meta::id(crash_id) as crash_id, meta::id(product_id) as product_id FROM attachments{suffix}"
        );
    ...
    pub async fn create(db: &Surreal<Any>, attachment: NewAttachment) -> Result<String, RepoError> {
        ...
        let _: Option<serde_json::Value> = db
            ...
            .bind(("crash_id", record_key(&attachment.crash_id)))
            .bind(("storage_path", attachment.storage_path.clone()))
            .bind(("product_id", record_key(&attachment.product_id)))
            .await
            .map_err(handle_surreal_error)?
            .take(0)
            .map_err(handle_surreal_error)?;
