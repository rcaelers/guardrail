...
export const load: PageServerLoad = async ({ locals, url, request, params }) => {
  ...
  // Only maintainers (and admins) can manage invitations.
  // Admins see all products; maintainers only see products they maintain.
  ...
  return {
    ...
    currentProductId: params.product,
    origin: url.origin
  };
};
...
export const actions: Actions = {
  create: async ({ request, locals }) => {
    ...
    try {
      await adapter.createInvitation({ is_admin, grants, expires_at: expires_at || null, max_uses: 1, to: email_to || undefined });
      return { ok: true };
    } catch (e) {
