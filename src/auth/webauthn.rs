...
use super::{error::AuthError, user::AuthenticatedUser};
use crate::{
    app_state::AppState,
    entity::{
        self,
...
pub async fn start_register(
    ...
    let exclude_credentials = Credential::find()
        ...
        .await?
        .iter()
        .map(|record| serde_json::from_value::<Passkey>(record.credential.clone()))
...
async fn update_passkeys(
    ...
    for cred in credentials {
        let mut passkey = serde_json::from_value::<Passkey>(cred.credential.clone())?;
        let updated = passkey.update_credential(&auth_result);
        if let Some(updated) = updated {
            if updated {
                let mut cred: entity::credential::ActiveModel = cred.into();
                cred.credential = Set(serde_json::to_value(&passkey)?);
